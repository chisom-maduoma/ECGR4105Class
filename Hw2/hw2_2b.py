#Homework 2, Problem 2b - Repeating Problem 1b but with normalization and standardization to stop the overfitting
import pandas as pd
import numpy as np
...
data = data.drop(columns=['price_per_sqft', 'furnishingstatus'], errors = 'ignore') 
...
x_norm_b = np.c_[np.ones((len(x_norm), 1)), x_norm] #add x0 = 1 to each instance
...
for alpha in learning_rates:
    ...
    plt.plot(train_loss, label=f'Raw Training Loss (alpha={alpha})')
    plt.plot(valid_loss, label=f'Raw Validation Loss (alpha={alpha})')
